...
sudo dpkg -x linux-image-current-rockchip64_*-trunk_arm64.deb "$tmpdir"
sudo dpkg -x armbian-firmware_*-trunk_all.deb "$tmpdir"
...
sudo mv $tmpdir/usr/lib/linux-image-*-rockchip64/* $tmpdir/boot/dtbs-lts
